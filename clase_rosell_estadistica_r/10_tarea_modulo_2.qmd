...
Primero tenemos que determinar si la variable es continua o categórica (variable es de tipo numérico o no). Luego, con if-else, la función calculará y devolverá la media o la moda.

```{r 10_funcion_ifelse}

funcion_ifelse_moda <- function(variable) {
  # Comprobar si la variable es numérica==continua
  if (is.numeric(variable)) {
    # Calcular y devolver la media
    return(mean(variable))
  } else {
    # Calcular y devolver la moda
...
2. Si la variable es numérica, calcula y devuelve su media con mean(variable).
3. Else=Si la variable no es numérica (categórica), la función calcula la moda. Esto se hace encontrando el valor único con la mayor frecuencia en la variable. 
4. La función itera sobre todos los valores únicos de la variable, cuenta la frecuencia de cada uno y mantiene el valor con la mayor frecuencia como la moda.
5. Finalmente, la función devuelve la moda para las variables categóricas.
...
Se estudia el dimorfismo sexual en una especie de gecko. Se mide la longitud hocico-cloaca en cm de
...
Para abordar la pregunta de si hay diferencias en la longitud hocico-cloaca entre machos y hembras de una especie de gecko, primero utilizaremos un análisis paramétrico, 

### Análisis paramétrico

Con análisis paramétrico ($t$ de muestras independientes) podemos comparar la media de dos grupos diferentes.

- Hipótesis nula (H_0): No hay diferencia en la media de longitud hocico-cloaca entre machos y hembras. 
μ machos =μ hembras
- Hipótesis alternativa (H1): Hay una diferencia en la media de longitud hocico-cloaca entre machos y hembras. H1 :μ machos != μ hembras
...
Efecto de una Droga en el Peso de Ratones
...
*Prueba Estadística*: Utilizar una prueba t de muestras pareadas.
...
Promedio de Días de Vacaciones en Oficinas del Gobierno
...
Función en R para Media o Moda
...
Una espiral se puede generar utilizando coordenadas polares, donde cada punto de la espiral tiene un ángulo y un radio que cambian de manera continua. La función para generar un patrón de espiral podría verse así:

```{r 10_funcion_espiral}

generar_espiral <- function(num_puntos = 1000, incremento = 0.1) {
  # Crear vectores vacíos para las coordenadas x e y
  x <- numeric(num_puntos)
  y <- numeric(num_puntos)
  
  # Generar puntos de la espiral
  for (i in 1:num_puntos) {
    # Calcular el ángulo y el radio
    angulo <- incremento * i
    radio <- incremento * i
    
    # Convertir de coordenadas polares a cartesianas
    x[i] <- radio * cos(angulo)
    y[i] <- radio * sin(angulo)
  }
  
  # Dibujar la espiral
  plot(x, y, type = 'l', xlab = 'X', ylab = 'Y', main = 'Patrón de Espiral')
}
...
Un patrón de Moiré se puede generar superponiendo dos conjuntos de líneas o patrones con ligeras variaciones entre ellos. 

```{r 10_funcion_moire}

generar_moire <- function(num_lineas = 50, desplazamiento = 0.1) {
  plot(1, type = "n", xlab = "", ylab = "", xlim = c(-10, 10), ylim = c(-10, 10))
  abline(h = -10:10, col = "grey")
  
  for (i in 1:num_lineas) {
    # Dibujar líneas horizontales con un pequeño desplazamiento
    abline(h = i * desplazamiento - 10, col = "blue")
    # Dibujar líneas verticales con un pequeño desplazamiento
    abline(v = i * desplazamiento - 10, col = "red")
  }
  
  title(main = "Patrón de Moiré")
}
